...
Лимит — **48 МБ** на файл. После загрузки файл раздаётся с `https://d1.endata.cx/data/games/{gameId}/{файл}` (мгновенно, без кэша — для разработки) и `https://cdn.endata.cx/data/games/{gameId}/{файл}` (кэш 24 ч — прод). Реализация: `encx.AdminUploadGameFile` (multipart), `zapolnyaka.UploadAssets([]AssetUpload)` (цикл + переавторизация при ErrSessionExpired), команда `cmd.RunAssets`.

### UUID-имена и плейсхолдеры `{{...}}`

Ассеты заливаются под случайными **UUID-именами** (`design.css` → `<uuid>.css`, расширение сохраняется для правильного Content-Type) — чтобы игроки не перебирали спойлерные имена. В контенте автор пишет `{{оригинальное_имя}}`, команда `go` подставляет полный URL `https://d1.endata.cx/data/games/{gid}/{uuid.ext}`.

- **Манифест** `assets/.manifest.json` (скрытый — сам не заливается): `map[оригинал]uploadName`. UUID **стабилен** (переиспользуется из манифеста при повторной заливке; если манифест потерять — новые uuid, старые файлы осиротеют). Манифест пересобирается из успешно залитых файлов (удалённые с диска выпадают).
- **Префикс `~`**: `~logo.png` → заливается как `logo.png` (без uuid, `~` снимается); ссылка `{{logo.png}}`. Коллизия `logo.png` + `~logo.png` → ошибка.
- **Подстановка** (`cmd.substituteAssets` в `go`): тело задания (`Body`), `Help` бонусов, тексты `Hints`/`PenaltyHints`. Регэксп `\{\{\s*([^{}]+?)\s*\}\}`; `{{x}}?v=2` → `.../<uuid>.ext?v=2`. Плейсхолдер без записи в манифесте → ошибка «сначала запусти assets».
- UUID генерится через `crypto/rand` (v4), без внешних зависимостей (`cmd.newUUID`).

Команда `assets` **не** трогает тела уровней; подстановка происходит только в памяти при `go` (исходные файлы с `{{...}}` не переписываются). Версию `?v=N` для сброса кэша поднимаешь вручную.
...
| `task` | опционально | бонусные типы | идёт в `txtTask` en.cx — «Задание» бонуса. **Видно игроку сразу** — в списке бонусов уровня, до ввода кода. Текст/HTML, плейсхолдеры `{{ассет}}` раскрываются. Только Go-версия. |
| `levels` | опционально | бонусные типы | на каких уровнях играет бонус — чекбоксы `level_<id>` формы бонуса (`rbAllLevels=0`) или `rbAllLevels=1` для `все`. Строка-спецификация: `"7"`, `"1-10"`, `">10"`, `">=10"`, `"<5"`, `"<=5"`, `"2,4,6-8"`, `"все"`/`"*"` (см. `config.ParseLevelSpec`). Без поля — только текущий уровень; с полем свой уровень не подразумевается. Номера резолвятся в DB-ID через `levelDbIds` при заливке. Мультиуровневые бонусы, созданные в текущем запуске, `cleanLevel` не удаляет (`createdBonuses`). Только Go-версия. |
...
**⚠ Что видит игрок ДО ввода кода:** `sectorName` (в списке секторов), `bonusName` и `task` (в списке бонусов), длительность/знак бонуса. **После ввода** — только `help`. Никогда не клади ответ или спойлер в `sectorName`/`bonusName`.
...
## Эмулятор play-страницы (команда `emu`, только Go-версия)

Пакет `go_app/internal/emu/` — локальный HTTP-сервер (`127.0.0.1:8090`, только stdlib), который рендерит уровень из конфигов так, как это делает движок. Команды: `emu [game.yml] [--port] [--dev] [--no-open]`, `snapshot [game.yml] [--name] [--send код] [--pid id]` (`cmd/emu_cmd.go`, `cmd/snapshot_cmd.go`), пункт TUI «🧪 Эмулятор уровней».

**Побайтовая точность (сверено 26.09.2026 по 40 состояниям уровней 6–15 игры 82460, tech.en.cx):** страница живёт по тому же пути, что в движке (`/gameengines/encounter/play/{gid}/`), CSS/JS грузятся с world.en.cx с `?ver=1.88.0.0` (`--offline` — из встроенной копии `static/engine/` с той же структурой путей; папка `jQuery/ui` на диске `jquery/ui`), строки движка — CRLF, тело задания вставляется как есть (LF). `templates/play.html` **генерируется** `tools/gen_template.py` из снимка уровня без блоков — не править руками (хвостовые табы/пробелы значимы, инструменты редактирования их срезают). Блоки — `render.go` (строки с точными табами). Факты движка, которые надо помнить:
- Ответ = POST на тот же URL, ответ 200 сразу (без PRG). Неверный ответ остаётся в `value` поля; в истории сверху `<li id="incorrect">Ответ или код неверный</li>` (верный — `<li class="color_correct">…верный</li>`), в записи `<span class="color_incorrect">` только в самом ответе на POST, на следующем GET — `class="incorrect"`; буквы неверного ответа обёрнуты в `<i>` («<i>точка</i>3»). Структура `<ul class="history">`: CRLF, пустая строка, [уведомление], пустая строка, записи (новые сверху), сразу `</ul>`.
- Секторбонус даёт две записи истории: сначала бонус (`color_bonus`/`color_pen` — у штрафа хвостовой таб после `</span>`), потом сектор (`color_correct`), поэтому сектор сверху. Два сектора с одним ответом — две записи.
- Прохождение уровня: ответом приходит страница следующего уровня с записью прошлого уровня и строкой `\t\t\t\t\t(N)` после времени; на следующем GET история пуста.
- Один сектор → без списка секторов и `RequiredSectorsCount=0` в JSON. Заголовок «На уровне N секторов » (хвостовой пробел) + `<span class="color_sec">(осталось закрыть K)</span>`, если K < N. ≤10 секторов — `div.cols.w100per`, больше — три `div.cols`. Закрытый сектор: `<span class="color_correct">ответ</span> <span class="color_sec">(dd.mm.yyyy hh:mm:ss <a href="/UserDetails.aspx?uid=…">login</a>)</span>`.
- Бонусы: `<h3 class="bonus_count">На уровне N бонусов </h3>` + `(Выполненные - K)` (K — взятые на этом уровне); невыполненный — `<h3 class="color_bonus"> ` + задание в `<p>` с шестью табами; выполненный — `<h3 class="color_correct">` + `(выполнен, награда 1 минуту)` (штраф — `color_pen`, «штраф 2 минуты») с тремя хвостовыми табами, `<div class="spacer_answer">`, `<span class="answer_bonus">? dd.mm hh:mm:ss login [ <span class="color_bonus">код</span> ]</span>`, help в `<p>`. Чужие бонусы (`levels`) идут первыми, если созданы раньше.
- Подсказки: показанная — `<h3>Подсказка N</h3><p>…</p>`; будущая — `<span class="color_dis"><b>Подсказка&nbsp;N</b>&nbsp;будет через&nbsp;<span class="bold_off" id="timeNNNNNNN">1 минуту 35 секунд</span><script>…new Timer({…"StartCounter":remain+1…}, … window.location = '/gameengines/encounter/play/{gid}/')…</script></span>`; единицы в JS короткие («дн/ч/м/с»), серверный текст — винительный падеж без нулевых частей (`ruDuration`).
- Штрафная подсказка: доступна → `<p>{comment}&nbsp;<a href="…?pact=2&amp;pid=ID">Взять подсказку (штраф 1 минуту)</a></p>`; `?pact=2` → страница подтверждения `За просмотр этой подсказки вам будет начислено <span>1 минуту<span> штрафного времени.&nbsp;<a …?pact=1&amp;pid=ID>Согласен</a>&nbsp;&nbsp;<a …>Отмена</a>` (незакрытый `<span>` — так у движка; состояние не сохраняется); `?pact=1` → открыта, `<p>текст</p>`. Ссылки EN/RU в футере несут query запроса, `lang` вставляется вторым.
- Автопереход: `<h3 class="timer"><strong>Автопереход</strong> на следующий уровень через&nbsp;<таймер>` + `(штраф&nbsp;2 минуты)`; в JSON `TimeoutAward` отрицательный.
- Имена уровней/секторов/бонусов HTML-кодируются как .NET HtmlEncode («» → `&#171;`/`&#187;`), кириллица не кодируется.
- Антибот: частые запросы → 302 на `NotHumanRequest.aspx` и временная блокировка сессии; логин привязан к User-Agent.

**Устройство:**
- `state.go` — состояние симуляции (`GameState`/`LevelState`, mutex, JSON в `<папка game.yml>/.emu-state.json`). Время уровня: `elapsed = Offset + (now − StartedAt)`, пауза копит в `Offset`. Подсказки — производные от времени; открытые штрафные — явное состояние; введённые бонусы — на всю игру (ключ `owner:idx`, мультиуровневые остаются введёнными).
- `match.go` — проверка ответа (trim + без регистра) одновременно по секторам и бонусам уровня (как движок без блокировки ответов).
- `rewrite.go` — `{{имя}}` → `/assets/имя`, `https://d1|cdn.endata.cx/data/games/<gid>/<файл>` → локальный файл (обратный манифест или basename), вырезание голых `<link>`.
- `model.go` — `BuildView` → `View` (всё посчитано для шаблона) + модель `encx.Level` в формате `?json=1`.
- `server.go` — маршруты: `GET|POST /gameengines/encounter/play/{gid}/` (POST отвечает страницей сразу, как движок; `?json=1` — модель; `?pid=&pact=1|2` — штрафные), `/play/{n}` (dev-переход на уровень), `/assets/{name}`, `/engine/*` (встроенные CSS/JS движка по реальным путям), `/emu/panel.js|capture.js`, `/api/level/{n}`, `POST /api/state/{n}` (`toggleCode`, `setElapsed`, `pause`, `autoAdvance`, `openHint`, `closeHint`, `reset`, `resetAll`, `goto`), `/api/mtime` (live reload), `OPTIONS|POST /api/snapshot` (CORS `*`, приём HTML/JSON с реальной страницы), `/snapshots/`. play.html рендерится `text/template` (html/template вырезает HTML-комментарии движка), значения кодируются `HTMLEncode` в Go.
- `templates/play.html` — сгенерированный скелет; `render.go` — блоки (`renderHistory`, `renderTimer`, `renderSectors`, `renderTask`, `renderHelps`, `renderPenalties`, `renderBonuses`), всё запросозависимое — в `Request` (уведомление, значение поля, перенесённые записи, подтверждение штрафной, query).
- `tools/gen_template.py` (снимок → шаблон), `tools/emudiff.py` (нормализующий побайтовый дифф), `tools/replay.py` (реплей сценария `steps.txt` на эмуляторе с диффом каждой страницы).
- `static/panel.js` — dev-панель в Shadow DOM (`#emu-panel-host` после `</html>`, z-index 10000 — выше тумана 9998), данные берёт из `/api/level/{n}`. Все действия → `POST /api/state` → `location.href = playPath` (GET, не reload — иначе повтор POST).
- Конфиги перечитываются `config.LoadAll` на каждый запрос — это и есть live reload. Фикстура для тестов: `internal/emu/testdata/game/`.

**Реальная play-страница en.cx (движок v1.88, снято с tech.en.cx):** `div.container > div.header (a.logo, img.gametype, h1>a#lblGameTitle, ul>li.refresh/.stats/.levelstats/.discuss/.design a#adaptive) + div.aside (form.answers_form[method=post]: hidden LevelId, LevelNumber; input#Answer[name="LevelAction.Answer" maxlength=4000]; input.submit[value=">>"]; label · div.pane#this > ul.history) + div.content (h2 «Уровень <span>N</span> из M» · … · #ordinary_helps · #penalty_helps · br · #bonuses) + div.footer`. CSS: `world.en.cx/css/v2/en/engines/engine.css`, `real.css` (`.content{margin-left:290px}`), `engine_adaptive.css` (≤768px), toastr. JS: jquery-1.6.2, jquery-ui core, `cons/main.js` (jScrollPane), `Timer.js` (`$.addTimer(id, sec, units, cb)`), `engines/misc.js` (jScrollPane на `.pane`, fade `#incorrect`), `common.js`. Cookie `view-mode` (desktop/mobile) меняет meta viewport; клик по `.color_bonus` копирует текст. Ответ = POST формы на тот же URL, полный перерендер. `?json=1` → GameModel; штрафная подсказка — GET `?pid=<HelpId>&pact=1`. Движок вырезает голые `<link>` из тела задания.

**Тестовые уровни** для сверки с движком — `data/emu/` (уровни 6–15 игры 82460): базовый, сектора 3/6, бонусы, комбо, подсказки, штрафные, автопереход, витрина дизайна, мультиуровневый бонус + `<script>` в help, стресс.

## Веб-интерфейс (команда `ui`, только Go-версия)

Пакет `go_app/internal/ui/` монтируется в маршрутизатор эмулятора (`emu.Server.Mux()`): `GET /ui/` — SPA (`static/index.html`, `app.css`, `app.js`, без сборки; `--dev` читает их с диска), `GET /ui/preview/{n}` — превью контента уровня (`emu.Server.Preview` — те же блоки, что на play-странице, синтетическое состояние «всё открыто», без шапки/формы). JSON API `/api/ui/`: `state` (игры из `data/*/game.yml`, уровни, ассеты с отметкой по манифесту, логин, последнее задание), `POST game` (переключить игру — `emu.Server.SetGame`), `level/{n}` (conf, codes, body, сырые тексты), `PUT level/{n}/codes|conf|raw/{which}` (запись через штатные загрузчики: невалидное не пишется; YAML пересобирается yaml.v3 — комментарии теряются), `preview/{n}/checks`, `POST run` (go/assets/validate/check/snapshot → задание), `jobs/{id}?from=N` (строки лога, по одному заданию одновременно; лог команд дублируется через `logger.Tee`, `validate`/`check` пишут в writer через `RunValidateTo`/`RunCheckTo`, `go` фильтрует уровни через `RunGoLevels`), `POST auth`, `POST game/new`, `POST level/new`, `snapshots`, `diff/{name}` (Go-порт emudiff: `emu.Diff`/`NormalizePage`). Команды приложения приходят через интерфейс `ui.Actions`, реализованный в `cmd/ui_cmd.go` (ui не может импортировать cmd). Фронт держит состояние в `S`, вкладки по hash, эмулятор — iframe канонического play-URL; «Ввести в эмуляторе» шлёт `toggleCode` в `/api/state/{n}` (ключи `s:<idx сектора>` / `b:<уровень>:<idx записи>`).
