...
- `ui <game> [--port 8090] [--offline] [--dev] [--no-open]` — веб-интерфейс на `/ui/` (пакет `go_app/internal/ui/`: JSON API `/api/ui/*`, задания с потоковым логом, редакторы codes/conf/task, превью, эмулятор в iframe; фронт — `static/index.html|app.css|app.js` без сборки). Команды для UI — `cmd/ui_cmd.go` (реализация `ui.Actions`)
