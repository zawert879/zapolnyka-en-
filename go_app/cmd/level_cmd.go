...
func buildCodesTemplate(format string) string {
	if format == "yml" {
		return strings.Join([]string{
			...
			"#   task        — задание бонуса (txtTask), HTML/{{ассеты}} — видно игроку ДО ввода кода",
			"#   levels      — на каких уровнях играет бонус: \"1-10\", \">10\", \"<=3\", \"2,4,6-8\", \"все\"",
			"#                 (по умолчанию — только текущий уровень; свой уровень не подразумевается)",
			"#   help        — подсказка/HTML/скрипт в txtHelp (для бонусных типов)",
			"# sectorName, bonusName и task видны игроку сразу (в списках секторов/бонусов уровня).",
			...
			"#     - хлеб",
			"#   time: 300              # -5 минут",
			"#   task: \"Найди хлеб <img src=\\\"{{1.jpg}}\\\">\"  # задание бонуса, видно до ввода",
			"#   levels: \"1-10\"         # играет на уровнях 1–10 (без поля — только на этом уровне)",
